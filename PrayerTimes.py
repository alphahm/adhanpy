...
def season_adjusted_morning_twilight(latitude: float, day: int, year: int, sunrise: datetime):
...
def season_adjusted_evening_twilight(latitude: float, day: int, year: int, sunset: datetime) -> datetime:
...
class PrayerTimes:
    def __init__(self, coordinates: Coordinates, date_components: DateComponents, calculation_parameters: CalculationParameters):
        ...
        prayer_date = datetime(date_components.year, date_components.month, date_components.day, tzinfo=timezone.utc)
        ...
        transit = None if time_components is None else time_components.date_components(date_components)

        time_components = TimeComponents.from_float(solar_time.sunrise)
        sunrise_components = None if time_components is None else time_components.date_components(date_components)

        time_components = TimeComponents.from_float(solar_time.sunset);
        sunset_components = None if time_components is None else time_components.date_components(date_components)
        ...
        error = False
        if transit is None or sunrise_components is None or sunset_components is None or tomorrow_sunrise_components is None:
            error = True
        ...
        if error is False:
            ...
            time_components = TimeComponents.from_float(solar_time.afternoon(calculation_parameters.madhab.get_shadow_length()))
            ...
            time_components = TimeComponents.from_float(solar_time.hour_angle(-calculation_parameters.fajr_angle, False))
        
            if time_components is not None:
                temp_fajr = time_components.date_components(date_components)

            if calculation_parameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE and coordinates.latitude >= 55:
                temp_fajr = sunrise_components + timedelta(seconds= -1 * int(night / 7000))

            
            night_portions = calculation_parameters.night_portions()

            #   final Date safe_fajr;
            if calculation_parameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE:
                safe_fajr = season_adjusted_morning_twilight(coordinates.latitude, day_of_year, prayer_date.year, sunrise_components)
            else:
                portion = night_portions.fajr
                night_fraction = int(portion * night / 1000)
                safe_fajr = sunrise_components + timedelta(seconds= -1 * int(night_fraction))
            ...
            try:
                ...
                temp_isha = temp_maghrib + timedelta(seconds=calculation_parameters.isha_interval * 60)
            except:
                timeComponents = TimeComponents.from_float(solar_time.hour_angle(-calculation_parameters.isha_angle, True))
                ...
                if calculation_parameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE and coordinates.latitude >= 55:
                    ...
                    temp_isha = sunset_components + timedelta(seconds= int(night_fraction))

                if calculation_parameters.method == CalculationMethod.MOON_SIGHTING_COMMITTEE:
                    safe_isha = season_adjusted_evening_twilight(coordinates.latitude, day_of_year, date_components.year, sunset_components)
                else:
                    portion = night_portions.isha
                    night_fraction = int(portion * night / 1000)

                    safe_isha = sunset_components + timedelta(seconds= int(night_fraction))
        ...
        if error is True or temp_asr is None:
            ...
            self.sunrise = rounded_minute(
                (temp_sunrise + timedelta(minutes= calculation_parameters.adjustments.sunrise))
                + timedelta(minutes= calculation_parameters.method_adjustments.sunrise)

            )
            
            self.dhuhr = rounded_minute(
                (temp_dhuhr + timedelta(minutes= calculation_parameters.adjustments.dhuhr))
            ...
            self.maghrib = rounded_minute(
                (temp_maghrib + timedelta(minutes= calculation_parameters.adjustments.maghrib))
