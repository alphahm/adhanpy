...
def ascending_lunar_node_longitude(T):
    # Equation from Astronomical Algorithms page 144
    term1 = 125.04452
    term2 = 1934.136261 * T
    term3 = 0.0020708 * (T ** 2)
...
def apparent_solar_longitude(T, L0):
...
def solar_equation_of_the_center(T, M):
...
def nutation_in_longitude(T, L0, Lp, Ω):
    # Equation from Astronomical Algorithms page 144
...
def nutation_in_obliquity(T, L0, Lp, Ω):
    # Equation from Astronomical Algorithms page 144
...
def mean_obliquity_of_the_ecliptic(T):
...
def apparent_obliquity_of_the_ecliptic(T, ε0):
    # Equation from Astronomical Algorithms page 165
    O = 125.04 - (1934.136 * T);
...
def approximate_transit(L, Θ0, α2):
    # Equation from page Astronomical Algorithms 102
    Lw = L * -1
    return normalize_with_bound((α2 + Lw - Θ0) / 360, 1)

def corrected_transit(m0, L, Θ0, α2, α1, α3):
    # Equation from page Astronomical Algorithms 102
    Lw = L * -1
...
def corrected_hour_angle(m0, h0, coordinates, afterTransit, Θ0, α2, α1, α3, δ2, δ1, δ3):
    ...
    term1 = math.sin(math.radians(h0)) - (math.sin(math.radians(coordinates.latitude)) * math.sin(math.radians(δ2)))
    ...
    term3 = h - h0
    term4 = 360 * math.cos(math.radians(δ)) * math.cos(math.radians(coordinates.latitude)) * math.sin(math.radians(H))
    Δm = term3 / term4
    return (m + Δm) * 24
